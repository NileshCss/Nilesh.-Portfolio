...
import { Mail, Eye, Reply, Trash2, Search, RefreshCw, Download, Check, X, CheckCircle, AlertTriangle } from "lucide-react";
...
export default function ContactMessagesPage() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      setFetchError(msg);
      toast.error("Unable to load messages: " + msg);
    }
    setLoading(false);
  }, [supabase, toast]);
  ...
  const markAsRead = async (id: string) => {
    ...
    const dbStatus = (targetMsg.status || 'Unread').toLowerCase();
  ...
  const normalizeMessage = (msg: Message) => {
    ...
    // Normalize status: handle lowercase from DB (e.g. 'unread' → 'Unread')
    const rawStatus = msg.status || (msg.is_read ? 'read' : 'unread');
    const statusMap: Record<string, 'Unread' | 'Read' | 'Confirmed'> = {
      unread: 'Unread',
      read: 'Read',
      confirmed: 'Confirmed',
      Unread: 'Unread',
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Error state */}
      {!loading && fetchError ? (
        <div className="flex flex-col items-center justify-center py-16 border border-red-200 dark:border-red-900/50 bg-red-50/50 dark:bg-red-950/10 rounded-2xl">
          <AlertTriangle className="w-10 h-10 text-red-400 dark:text-red-500 mb-3" />
          <p className="text-sm font-semibold text-red-600 dark:text-red-400 mb-1">Unable to load messages.</p>
          <p className="text-xs text-red-500/70 dark:text-red-400/60 mb-4">{fetchError}</p>
          <button
            onClick={fetchMessages}
            className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-xl transition-all flex items-center gap-2"
          >
